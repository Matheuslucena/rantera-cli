...
class Tarefa {
    ...
    /**
     * Pesquisa tarefa e copia o titulo da tarefa selecionada para o clipboard
     *
     */
    async copiarTitulo() {
        printPlanejamentoAtual(this.getPlanejamentoAtual());
        const { tarefa } = await formPesquisarTarefa(this.listaTarefas(), true);
        const tituloTratado = _.snakeCase(_.deburr(tarefa));
        console.info(`${chalk.white('O titulo:')} ${tituloTratado} ${chalk.green('[Foi Copiado]')}`);
        clipboardy.writeSync(tituloTratado);
    }
